...
const GenerateAssessmentOutputSchema = z.object({
  ...
  selectedImageSummary: z.string().describe("A summary of the selected images, grouped by category, returned as a JSON string. The key is the category name and the value is an array of image descriptions."),
  contributingFactorsSummary: z.string().describe("A summary of the selected contributing factors, grouped by category, returned as a JSON string. The key is the category name and the value is an array of selected tags."),
});
...
- **Summaries**: Create summaries for both 'selectedImageSummary' and 'contributingFactorsSummary' by grouping the user's selections by their 'category' field. The result for these fields must be a valid JSON string where keys are the category names and values are arrays of the descriptions/tags.
